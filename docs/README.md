...
| 入口 | 用途 | 当前状态 |
| --- | --- | --- |
| [企业智能体记忆系统评估](enterprise-memory-system-evaluation.md) | 当前范围、风险、下一 Gate | Task 3 handler/route + source-build assets Passed；服务/Mock/API/TUI Not Run |
...
| [Task 3 Anthropic platform routes Passed](reproduction/2026-08-11-task3-anthropic-platform-routes-passed.md) | 三平台 literal route、source 保真、session/privacy 与新 Proxy image | Runtime Passed（handler/route + build/assets only） |
...
| [集成实验 SOP](../tests/integration/README.md) | 后续静态验证与受控运行入口 | Task 3 Passed；Mock/客户端 Gate 未运行 |

> **状态：Historical** — 本文记录 2026-08 的四 Agent / Docker 实验评估，不是当前方案、运行状态或执行 SOP。
...
本文原为该实验线的主评估入口。设计背景见当前的[企业记忆方案](../../design/enterprise-memory-design.md)；实验规格、决策与复现证据分别保存在本归档目录的 [specifications/](specifications/)、[decisions/](decisions/) 和 [reproduction/](reproduction/)。原集成 SOP 与执行代码已删除。
...
当时的镜像、Mock Gate、Docker Claude 与 Windows Claude 结论仅保留为历史证据，不能据此重建或宣称当前运行能力。对应记录见 [reproduction/](reproduction/)；原集成说明与执行代码已随仓库角色收敛删除。

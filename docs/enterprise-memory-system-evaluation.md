...
| Stage 1 Mock identity/share/isolation/leak | Blocked / claude-cli-nonzero-before-mock | `7186820a` coordinator exit 0，四固定步骤完成；canonical 分类为 Claude child nonzero、continuity ok、all Mock request deltas 0。它只收窄到 Mock 前，不能区分 settings/CLI args/auth/onboarding/capture/Proxy-before-Mock；后续业务 Gate Not Run |

...
> **状态：Snapshot**
> **来源：** [TencentCloud/TencentDB-Agent-Memory](https://github.com/TencentCloud/TencentDB-Agent-Memory)，`feat/server_team @ 97f9465`（2026-08-15）。
> **检查日期：** 2026-08-18；仅作源码与仓库文档静态调研，未启动服务。
>
> 本文的 `文件:行号` 均指向当时的外部源码快照。根仓库不再携带该源码工作树；2026-08-10 fork 参考仅保留在[历史实验归档](../../history/integration-lab-2026-08/references/tencentdb-agent-memory-fork-2026-08-10.md)。本文回答部署方式、团队搭建、资产、角色、成员/Agent/Task 关系和知识更新入口，不代表上游当前状态。
>
...
- 部署：`deploy/global-images` 在线镜像路径已相当成熟（随机 admin key、健康检查、一行命令接入、purge 清理）。本报告基于的 `97f9465` 未做运行验证；已归档的 2026-08 Docker 实验也只证明其各自的历史 pin，不能外推到此快照。该分支处于 Beta 快速迭代期，Release、CHANGELOG 与 Docker tag 不完全同步，引用时应固定 commit 或镜像 digest，不要依赖 `latest`。

...
- **Fact**：Task 5 auth service-token fix 已在 `9e456a5...` 通过 focused `3/3`、fresh full `38/38` suites / `276/276` tests、exact six baseline typecheck errors与最终 review `CLEAN`；replacement Proxy 与 root `bfb3839a` 的 tools fixture fix/replacement image `sha256:8ca1a2a82a585ecf477577db2308c09348b4c7a6ff6022693f3653ed20004d81` 保持 **Ready（build/assets only）**。历史 [preflight Blocked](docs/reproduction/2026-08-11-task5-mock-20260811-preflight-7c1a9e2b-blocked.md)、[protocol/leak Blocked](docs/reproduction/2026-08-11-task5-mock-20260811-fixed-8d4802d5-protocol-leak-blocked.md)、[authfix fixture Blocked](docs/reproduction/2026-08-11-task5-mock-20260811-authfix-2eae9df1-tool-fixture-blocked.md) 与 [toolsfix Claude write Blocked](docs/reproduction/2026-08-11-task5-mock-20260811-toolsfix-7e1031af-claude-write-blocked.md) 均 append-only。最后一个 formal run 已通过 protocol/leak `24/24` 与 management，但在 step 8 Claude write fail-stop；aggregate/Core/version 观测只收窄到 Claude `-p`/capture/CLI-before-Proxy，状态为 **Blocked / diagnostic pending**，不得虚构根因。后续两写、六读、final、真实 headless、TUI 与真实模型仍为 **Not Run**。

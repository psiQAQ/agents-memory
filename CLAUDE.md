...
- **Fact**：Task 5 auth service-token fix 已在 `9e456a5...` 通过 focused `3/3`、fresh full `38/38` suites / `276/276` tests、exact six baseline typecheck errors与最终 review `CLEAN`；replacement Proxy `local/refine-memory-proxy:9e456a5-auth-fix@sha256:55fedae3f6a3a0a45ac8be45461d8cab23c52f11cc089c1c1e54c7d236de764b` 已完成唯一 build 与离线 image/assets Gate，状态为 **Ready（build/assets only）**。root TDD 将 gitlink/Compose 更新到该 exact input，并保留 launcher 的 request-local credential 合同；root Node 151/151、Compose config 7/7 Passed。旧 deterministic Mock run 仍是 append-only Blocked 历史；replacement image 下的新唯一 tuple rerun 为 **Not Run**，可在获得运行授权后按 tracked launcher 执行。完整 Mock Gate 通过前，真实 CLI headless、三写六读、live outsider/management、final oracle、TUI、Paid/真实模型与 Codex Stage 2 均未通过或未运行，不得提前进入 TUI/real。

...
- **Fact**：Task 5 auth service-token fix 已在 `9e456a5...` 通过 focused `3/3`、fresh full `38/38` suites / `276/276` tests、exact six baseline typecheck errors与最终 review `CLEAN`；replacement Proxy `local/refine-memory-proxy:9e456a5-auth-fix@sha256:55fedae3f6a3a0a45ac8be45461d8cab23c52f11cc089c1c1e54c7d236de764b` 已完成唯一 build 与离线 image/assets Gate。root `bfb3839a` 的 tools fixture fix 以 mock `8/9` RED→`9/9` GREEN、runner `43/43`、root Node `152/152`、Compose config `7/7` 通过 review（Critical/Important 均 0）；唯一 tools replacement image 为 `sha256:8ca1a2a82a585ecf477577db2308c09348b4c7a6ff6022693f3653ed20004d81`，状态为 **Ready（build/assets only）**。历史 [preflight Blocked](docs/reproduction/2026-08-11-task5-mock-20260811-preflight-7c1a9e2b-blocked.md)、[protocol/leak Blocked](docs/reproduction/2026-08-11-task5-mock-20260811-fixed-8d4802d5-protocol-leak-blocked.md) 与 [authfix fixture Blocked](docs/reproduction/2026-08-11-task5-mock-20260811-authfix-2eae9df1-tool-fixture-blocked.md) 均 append-only；[tools replacement build](docs/reproduction/2026-08-11-task5-tools-fixture-replacement-build-passed.md) 不改变其结论。全新 deterministic Mock rerun、TUI 与真实模型仍为 **Not Run**。完整 Mock Gate 通过前，真实 CLI headless、三写六读、live outsider/management、final oracle 与 Codex Stage 2 均未通过或未运行，不得提前进入 TUI/real。

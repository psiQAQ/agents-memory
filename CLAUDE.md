...
- **Fact**：Tencent upstream base 固定为 default `feat/server_team@0a568c328ea1aae3f22ed3656e7900da7ea565c1`；active fork/gitlink 为 reviewed Task 5 session-identity fix `codex/four-agent-memory-upstream@2de58c2f656978cfe310e3ac3ade085d8096f83b`。`origin/codex/four-agent-memory-upstream` 仍为 `38ced16f46fed640bcb7360fb1ca45f9f9918628`，所以 active gitlink 是 local-only、fresh clone 暂不可取得；未经单独授权不得 push。upstream 前移时仍须先审查增量，不能静默更新。
...
- **Fact**：Task 5 产品 session-identity fix 已在 `2de58c2...` 通过 19 files / 298 tests、exact six baseline typecheck errors、formal Spec/Quality PASS 与最终 CLEAN；固定 Proxy 为 `local/refine-memory-proxy:2de58c2-l1-fix@sha256:be847074bd63e34ba85b1eee8638cd7d2457d3617d85ad880d36d71efea69fcd`。root TDD 将 gitlink/Compose 更新到该 exact input，并让 launcher 从同一私有 bundle key 同时覆盖 `MEMORY_USER_KEY` 与 request-local `TDAI_MEMORY_USER_KEY`；root Node 150/150、Compose config 7/7 Passed。tools 与 Claude/OpenCode/Pi image 已串行各 build 一次，version/help、UID 10001、`/client-evidence` 10001:10001 可写与 headless assets 为 **Runtime Passed（build/assets only）**。服务、Proxy→Mock、真实 CLI headless、三写六读、live outsider/management、final oracle、TUI、Paid/真实模型与 Codex Stage 2 均为 **Not Run**。

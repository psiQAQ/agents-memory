...
**Fact**：Task 5 auth service-token fix、root `bfb3839a` tools fixture fix 与 Proxy/tools replacement images 均为 Ready（build/assets only）。历史 [preflight Blocked](../../docs/reproduction/2026-08-11-task5-mock-20260811-preflight-7c1a9e2b-blocked.md)、[protocol/leak Blocked](../../docs/reproduction/2026-08-11-task5-mock-20260811-fixed-8d4802d5-protocol-leak-blocked.md)、[authfix fixture Blocked](../../docs/reproduction/2026-08-11-task5-mock-20260811-authfix-2eae9df1-tool-fixture-blocked.md) 与 [toolsfix Claude write Blocked](../../docs/reproduction/2026-08-11-task5-mock-20260811-toolsfix-7e1031af-claude-write-blocked.md) 继续 append-only；后者 protocol/leak `24/24` Passed、management ok，但 step 8 `claude-write` fail-stop。aggregate/Core/version 只收窄 Claude `-p`/capture/CLI-before-Proxy 边界，状态为 **Blocked / diagnostic pending**。后续两写、六读、final、真实 headless、TUI 与真实/Paid 模型均为 Not Run。既有 Windows + Claude 命令、Compose 路径和运行证据是 Legacy，不能直接复用到四 CLI 路线。
...
完整前置链见 [privacy/build Passed](../../docs/reproduction/2026-08-11-task5-proxy-privacy-build-passed.md)、[pre-runtime round 1 erratum/RED](../../docs/reproduction/2026-08-11-task5-pre-runtime-review-round1-erratum.md)、[root static Passed](../../docs/reproduction/2026-08-11-task5-pre-runtime-review-round1-root-static-passed.md)、[preflight Blocked](../../docs/reproduction/2026-08-11-task5-mock-20260811-preflight-7c1a9e2b-blocked.md)、[protocol/leak Blocked](../../docs/reproduction/2026-08-11-task5-mock-20260811-fixed-8d4802d5-protocol-leak-blocked.md)、[authfix fixture Blocked](../../docs/reproduction/2026-08-11-task5-mock-20260811-authfix-2eae9df1-tool-fixture-blocked.md)、[toolsfix Claude write Blocked](../../docs/reproduction/2026-08-11-task5-mock-20260811-toolsfix-7e1031af-claude-write-blocked.md)、[auth replacement image Passed](../../docs/reproduction/2026-08-11-task5-auth-service-token-replacement-image-passed.md) 与 [tools replacement build Passed](../../docs/reproduction/2026-08-11-task5-tools-fixture-replacement-build-passed.md)。live 顺序固定为 protocol/leak → management/outsider → 三次顺序写入 → 六次有序跨 owner 读取 → final oracle；本次 step 8 failure 不把 failure 后服务 healthy 扩写为 Claude write 或任何后续 business step Passed。下一 Gate 为 Claude write diagnostic，不重跑/TUI/real。

## Task 5 Mock runtime 启动器（Claude write Blocked；diagnostic pending）

历史固定 run `task5-mock-20260811-preflight-7c1a9e2b`、`task5-mock-20260811-fixed-8d4802d5`、`task5-mock-20260811-authfix-2eae9df1` 与 `task5-mock-20260811-toolsfix-7e1031af` 均保持 append-only Blocked；最后一个 run 的 protocol/leak `24/24` 和 management 已通过，但在 step 8 Claude write fail-stop。其 aggregate/Core/version 观测只用于收窄 Claude `-p`/capture/CLI-before-Proxy，不能作为根因或后续 Gate 结论。当前 project/evidence 必须保留；下一 Gate 是最小无 secret Claude write diagnostic，不重跑/TUI/real。
...
4. **Task 5（Claude write Blocked；diagnostic pending）**：四次 deterministic Mock failures 保持 append-only；auth/fixture fixes 与 replacement images 仍为 Ready（build/assets only）。最新 run 已通过 protocol/leak 和 management、在 step 8 fail-stop；后续两写、六读、outsider、final oracle 与真实 headless 未运行，不得提前进入 TUI/real。

...
**Fact**：Task 5 auth service-token fix、root `bfb3839a` tools fixture fix 与 Proxy/tools replacement images 均为 Ready（build/assets only）。历史 Blocked records 继续 append-only。`08c403c6` 保持 host-timeout-during-up Blocked，`a72ae725` 保持 host-native-stderr-during-up Blocked；其 config-init/healthy 盘点不是业务通过。tracked Node coordinator 已按无 drift 的 `spawnSync`、bounded output 与 canonical JSON 合同完成 focused `22`、root `192`、Compose `7` 及独立 C/I/M `0` 静态验证。新唯一 `7186820a` tuple 的 labels、evidence/path、port、images 与 diagnostic overlay preflight 已通过，状态仅为 **Ready / Not Run**；如获运行授权，仅一次固定 runtime/canonical JSON diagnostic，禁止 retry/reset/finalize/TUI/real。后续两写、六读、final、真实 headless、TUI 与真实/Paid 模型均为 Not Run。既有 Windows + Claude 命令、Compose 路径和运行证据是 Legacy，不能直接复用到四 CLI 路线。
...
完整前置链见 [privacy/build Passed](../../docs/reproduction/2026-08-11-task5-proxy-privacy-build-passed.md)、[root static Passed](../../docs/reproduction/2026-08-11-task5-pre-runtime-review-round1-root-static-passed.md)、[08c403c6 diagnostic Ready](../../docs/reproduction/2026-08-11-task5-diag-claude-p-20260811-08c403c6-ready.md)、[08c403c6 host-timeout Blocked](../../docs/reproduction/2026-08-11-task5-diag-claude-p-20260811-08c403c6-host-timeout-during-up-blocked.md)、[a72ae725 diagnostic Ready](../../docs/reproduction/2026-08-11-task5-diag-claude-p-20260811-a72ae725-ready.md)、[a72ae725 host-native-stderr Blocked](../../docs/reproduction/2026-08-11-task5-diag-claude-p-20260811-a72ae725-host-native-stderr-during-up-blocked.md) 与 [7186820a diagnostic Ready](../../docs/reproduction/2026-08-12-task5-diag-claude-p-20260812-7186820a-ready.md)。下一 Gate 仅在获得运行授权后，使用该新 tuple 执行一次 fixed runtime/canonical JSON diagnostic；不得从当前 preflight 或历史 health inventory 推断 JSON/classification/root cause，不 retry/TUI/real。

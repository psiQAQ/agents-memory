...
**Fact**：Task 5 auth service-token、Mock fixture、Claude EROFS/tmpfs 与 OpenCode fixed-title fixes/replacement images 均已 Passed。`opentitle-4f056ee6` 的单次 full Mock launcher在 fixed step 9 fail-stop并已 exact cleanup；typed diagnostic `89398d1d` 已证明 render/spawn/capture/CLI code0 与 exact single Mock operation并已 cleanup。normal headless fixed-phase diagnostic 已在 `cbd1a27...` 完成 TDD、focused `47/47`、root `254/254`、Compose `7/7 + 1/1` 与 `C0/I0/M0` review；全新 `a666e597` tuple 已通过 freshness preflight，状态为 Ready/Not Run。业务 Gate 仍是 Blocked；完整三写六读/final、TUI 与真实/Paid 模型仍无通过证明。
...
Root full Node 为 254/254，base + `mock`、`real`、`claude`、`opencode`、`pi`、`management` Compose config 为 7/7，normal headless diagnostic overlay config 为 1/1。normal headless fixed-phase diagnostic 先以 missing module/export RED，再以 focused `47/47` GREEN；独立 review 为 `C0/I0/M0`。这些均为 **Static/contract Passed**，不是业务运行。
...
完整前置链见 [privacy/build Passed](../../docs/reproduction/2026-08-11-task5-proxy-privacy-build-passed.md)、[OpenCode fixed-title replacement image Passed](../../docs/reproduction/2026-08-12-task5-opencode-title-replacement-image-passed.md)、[opentitle-4f056ee6 OpenCode write Blocked](../../docs/reproduction/2026-08-12-task5-mock-20260812-opentitle-4f056ee6-opencode-write-blocked.md)、[OpenCode typed diagnostic `89398d1d` Passed](../../docs/reproduction/2026-08-12-task5-diag-opencode-20260812-89398d1d-passed.md) 与 [normal headless phase diagnostic `a666e597` Ready](../../docs/reproduction/2026-08-12-task5-diag-opencode-headless-20260812-a666e597-ready.md)。不得把 diagnostics、single Mock request、static contract 或 build/assets 扩写为完整三写六读/final/TUI/real 通过。

## Task 5 Mock runtime 启动器（OpenCode normal headless phase diagnostic Ready）

历史固定 Mock/diagnostic runs 均保持 append-only。`e83748e2`、`stepfix-2df660d8`、`opentitle-4f056ee6` 与 typed diagnostic `89398d1d` 均已 exact cleanup，不得复用或 retry。全新 `a666e597` 仅用于 normal OpenCode headless phase diagnostic；它会先执行一次正常 Claude write 形成 prior-operation baseline，再让 OpenCode 真实经过 client、exact aggregate verifier 与 atomic evidence publish 路径。

下一 Gate 只允许对 `a666e597` 执行一次 tracked `tests/integration/tools/run-task5-opencode-headless-diagnostic.mjs`。禁止 build、retry、复用旧 tuple、直接重跑 full Mock、读取 raw child/log/evidence、进入 TUI 或真实模型。只有 freshness 已通过且本次运行创建 exact project 后，才对该 project 使用对应 base/mock/claude/opencode/diagnostic overlays 执行 `down --volumes --remove-orphans`；preflight/label collision 失败必须保留现场审计。不得使用 `--rmi` 或全局 prune。下方 full Mock 命令只保留为合同参考，当前 Blocked 状态不授权执行。
...
4. **Task 5（Blocked；normal headless phase diagnostic Ready/Not Run）**：`89398d1d` 已证明 typed launch 与 exact single Mock operation；`a666e597` 只允许单次验证普通 headless verifier/evidence publish，不得进入 full Mock/TUI/real。

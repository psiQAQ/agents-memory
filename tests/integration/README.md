...
**Fact**：Task 5 auth service-token fix、root `bfb3839a` tools fixture fix 与 Proxy/tools replacement images 均为 Ready（build/assets only）。历史 Blocked records 继续 append-only。`08c403c6` 第一次宿主动作在 workload 前停止，修正后的首个 workload attempt 在 Compose up 被 1 秒 host timeout 中止；evidence 0 files，bootstrap/config/headless 未执行，状态为 **Blocked / host-timeout-during-up**，不得声明 JSON/classification/root cause。新唯一 `a72ae725` tuple 的 labels、evidence/path、port、images 与 diagnostic overlay preflight 已通过，状态仅为 **Ready / Not Run**；如获运行授权，仅一次 long-lived asynchronous host execution/direct structured canonical JSON parser diagnostic，禁止 retry/reset/finalize/TUI/real。后续两写、六读、final、真实 headless、TUI 与真实/Paid 模型均为 Not Run。既有 Windows + Claude 命令、Compose 路径和运行证据是 Legacy，不能直接复用到四 CLI 路线。
...
完整前置链见 [privacy/build Passed](../../docs/reproduction/2026-08-11-task5-proxy-privacy-build-passed.md)、[root static Passed](../../docs/reproduction/2026-08-11-task5-pre-runtime-review-round1-root-static-passed.md)、[08c403c6 diagnostic Ready](../../docs/reproduction/2026-08-11-task5-diag-claude-p-20260811-08c403c6-ready.md)、[08c403c6 host-timeout Blocked](../../docs/reproduction/2026-08-11-task5-diag-claude-p-20260811-08c403c6-host-timeout-during-up-blocked.md) 与 [a72ae725 diagnostic Ready](../../docs/reproduction/2026-08-11-task5-diag-claude-p-20260811-a72ae725-ready.md)。下一 Gate 仅在获得运行授权后，使用该新 tuple 进行一次 long-lived async exec/direct structured canonical JSON parser 的 Claude write diagnostic；不得从当前 preflight 或 timeout 推断 JSON/classification/root cause，不 retry/TUI/real。

...
**Fact**：Task 5 auth service-token fix、root `bfb3839a` tools fixture fix 与 Proxy/tools replacement images 均为 Ready（build/assets only）。历史 Blocked records 继续 append-only。`08c403c6` 保持 host-timeout-during-up Blocked。`a72ae725` 的 preflight 保持 Ready 历史，但单次 workload attempt 在 Compose `up` 被 Windows PowerShell 5 将正常 Docker stderr progress 转换为 terminating `NativeCommandError` 中止，状态为 **Blocked / host-native-stderr-during-up**。evidence 为 0 files；bootstrap/config/headless/JSON/classification 未执行。failure-time 的 config-init exit 0 与四个服务 healthy 仅为资源盘点，不得写成业务通过。当前 tuple 不复用，exact cleanup 已获授权且 host evidence 保留；先以 `spawnSync`、bounded stdout 与 direct structured canonical JSON parser 为 tracked Node coordinator 写 TDD，再使用新 tuple。后续两写、六读、final、真实 headless、TUI 与真实/Paid 模型均为 Not Run。既有 Windows + Claude 命令、Compose 路径和运行证据是 Legacy，不能直接复用到四 CLI 路线。

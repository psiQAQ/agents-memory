...
**Fact**：Task 5 auth service-token fix、root `bfb3839a` tools fixture fix 与 Proxy/tools replacement images 均为 Ready（build/assets only）。历史 Blocked records 继续 append-only。`08c403c6` 保持 host-timeout-during-up Blocked，`a72ae725` 保持 host-native-stderr-during-up Blocked；其 config-init/healthy 盘点不是业务通过。`7186820a` 的 preflight 保持 Ready 历史，但单次 fixed runtime 的 coordinator exit 0/四固定步骤只产生 canonical `claude-cli-nonzero-before-mock` 分类：Claude child 在任何 Mock-observed request 前 nonzero，continuity ok、all deltas 0。不得区分或归因为 settings、CLI args、auth、onboarding、capture 或 Proxy-before-Mock。当前 project 保留等待 exact cleanup，host evidence 保留且不读取原文；先为 tracked bounded phase/category 写 TDD，再使用新 tuple。后续两写、六读、final、真实 headless、TUI 与真实/Paid 模型均为 Not Run。既有 Windows + Claude 命令、Compose 路径和运行证据是 Legacy，不能直接复用到四 CLI 路线。

...
**Fact**：Task 5 auth service-token、Mock fixture、Claude EROFS/tmpfs 与 OpenCode fixed-title fixes/replacement images 均已 Passed。`opentitle-4f056ee6` 的单次 full Mock launcher 在 fixed step 9 fail-stop 并已 exact cleanup。OpenCode typed diagnostic 已在 `c163b6c...` 完成 TDD、focused `60/60`、root `239/239`、Compose `7/7 + 1/1` 与 `C0/I0/M0` review；全新 `89398d1d` tuple 单次 coordinator exit `0`，证明 render/spawn/capture/CLI code0、预期 operation/main 精确一次且无 unexpected/leak/drop/truncation，随后 exact cleanup 为 `0/0/0` 并保留 active images `7/7`。该诊断仍不证明普通 headless exact verifier/evidence publish，当前业务 Gate 是 Blocked；下一 Gate 是 TDD normal headless fixed-phase diagnostic，完整三写六读/final、TUI 与真实/Paid 模型仍无通过证明。
...
完整前置链见 [privacy/build Passed](../../docs/reproduction/2026-08-11-task5-proxy-privacy-build-passed.md)、[EROFS/tmpfs fix Passed](../../docs/reproduction/2026-08-12-task5-claude-erofs-tmpfs-fix-passed.md)、[113ca669 diagnostic Passed](../../docs/reproduction/2026-08-12-task5-diag-claude-p-20260812-113ca669-passed.md)、[OpenCode fixed-title replacement image Passed](../../docs/reproduction/2026-08-12-task5-opencode-title-replacement-image-passed.md)、[opentitle-4f056ee6 OpenCode write Blocked](../../docs/reproduction/2026-08-12-task5-mock-20260812-opentitle-4f056ee6-opencode-write-blocked.md)、[opentitle-4f056ee6 exact cleanup](../../docs/reproduction/2026-08-12-task5-mock-20260812-opentitle-4f056ee6-exact-cleanup-passed.md)、[OpenCode typed diagnostic `89398d1d` Passed](../../docs/reproduction/2026-08-12-task5-diag-opencode-20260812-89398d1d-passed.md) 与 [exact cleanup](../../docs/reproduction/2026-08-12-task5-diag-opencode-20260812-89398d1d-exact-cleanup-passed.md)。下一步是先按 TDD 固定 normal headless 的 verifier/evidence publish 阶段输出；不得将 typed diagnostic、steps 1–8、单次 Mock request、static contract 或 replacement build/assets 扩写为完整三写六读/final/TUI/real 通过。

## Task 5 Mock runtime 启动器（OpenCode normal headless fixed-phase diagnostic pending）

历史固定 Mock/diagnostic runs 均保持 append-only。`e83748e2`、`stepfix-2df660d8`、`opentitle-4f056ee6` 与 typed diagnostic `89398d1d` 均已 exact cleanup，不得复用或 retry。`opentitle-4f056ee6` 的唯一 launcher 在 fixed step 9 fail-stop；`89398d1d` 随后的单次 typed diagnostic 已证明 OpenCode render/spawn/capture/CLI code0、expected operation/main 精确一次且 aggregate clean，但没有执行普通 headless 的 exact verifier/evidence publish。

下一 Gate 只允许先按 TDD 给 normal headless 增加固定阶段诊断，区分 exact verifier 与 evidence publish；测试、review 与全新 tuple preflight 通过前，不执行新的 Docker workload。禁止复用旧 tuple、直接重跑 full Mock、读取 raw child/log/evidence、进入 TUI 或真实模型。任何后续 exact project 都必须遵守 freshness、append-only result 与精确 cleanup 规则，不得使用 `--rmi` 或全局 prune。下方 full Mock 命令只保留为合同参考，当前 Blocked 状态不授权执行。
...
4. **Task 5（Blocked；OpenCode typed diagnostic Passed/cleaned）**：`89398d1d` 已证明 render/spawn/capture/CLI 与 exact single Mock operation，但未覆盖 normal headless verifier/evidence publish；下一步只允许先 TDD fixed-phase diagnostic，不得进入 full Mock/TUI/real。

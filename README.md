...
| 阶段 | 客户端 | 协议与状态 |
| --- | --- | --- |
| Stage 1 | Claude Code `2.1.226`、OpenCode `1.18.16`、Pi `0.84.1` | 原生 route、Task 4 client assets、Task 5 harness 与 Proxy privacy/build-assets Passed；Docker 业务运行 Not Run |
...
**Boundary**：Task 5 root harness 的 104/104 Node tests 与 6/6 Compose config 只证明静态契约；产品 `d6afcd8` 的 113/113 tests、独立 review 和固定 Proxy `sha256:d79751b6...` 只证明 product privacy 与 build/runtime assets。header allowlist、credential origin/redirect、结构化 telemetry sinks 和 active/auxiliary/injection diagnostics 已进入产品级测试，但 live Proxy→Mock、真实 CLI headless、三写六读、outsider/management、TUI 与真实模型仍是 **Not Run**。旧 Windows + Claude 方案及其运行记录均归档为 **Legacy**。

## 起点

1. 阅读 [项目指令](CLAUDE.md)、[文档索引](docs/README.md) 和 [当前评估](docs/enterprise-memory-system-evaluation.md)。
2. 阅读 [四 Docker CLI 实施计划](docs/superpowers/plans/2026-08-10-four-docker-cli-memory.md)、[架构 ADR](docs/decisions/2026-08-10-four-docker-cli-baseline.md)、[Task 5 privacy RED](docs/reproduction/2026-08-11-task5-proxy-privacy-hardening-red.md) 与 [Task 5 privacy/build Passed](docs/reproduction/2026-08-11-task5-proxy-privacy-build-passed.md)。

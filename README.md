...
| 阶段 | 客户端 | 协议与状态 |
| --- | --- | --- |
| Stage 1 | Claude Code `2.1.226`、OpenCode `1.18.16`、Pi `0.84.1` | 原生 route、auth fix/review 与 replacement Proxy image Ready；deterministic Mock rerun Not Run |
...
**Boundary**：Task 5 root 151/151 Node tests 与 base + 六 overlays 的 7/7 Compose config 证明静态合同；Proxy 固定为 `local/refine-memory-proxy:9e456a5-auth-fix@sha256:55fedae3f6a3a0a45ac8be45461d8cab23c52f11cc089c1c1e54c7d236de764b`，replacement build 状态为 **Ready（build/assets only）**。旧 deterministic Mock Blocked 记录保持 append-only；replacement image 下的新唯一 tuple rerun 为 **Not Run**，可在获得运行授权后按 tracked launcher 执行。完整 Mock Gate、真实 CLI headless、三写六读、outsider/management live oracle、final oracle、TUI、真实模型与 Codex Stage 2 均未通过或未运行；不得提前进入 TUI/real。旧 Windows + Claude 方案及其运行记录均归档为 **Legacy**。

## 起点

1. 阅读 [项目指令](CLAUDE.md)、[文档索引](docs/README.md) 和 [当前评估](docs/enterprise-memory-system-evaluation.md)。
2. 阅读 [四 Docker CLI 实施计划](docs/superpowers/plans/2026-08-10-four-docker-cli-memory.md)、[架构 ADR](docs/decisions/2026-08-10-four-docker-cli-baseline.md)、[Task 5 privacy/build Passed](docs/reproduction/2026-08-11-task5-proxy-privacy-build-passed.md)、[pre-runtime round 1 erratum](docs/reproduction/2026-08-11-task5-pre-runtime-review-round1-erratum.md)、[root static Passed](docs/reproduction/2026-08-11-task5-pre-runtime-review-round1-root-static-passed.md)、[历史 protocol/leak Blocked](docs/reproduction/2026-08-11-task5-mock-20260811-fixed-8d4802d5-protocol-leak-blocked.md) 与 [auth replacement image Passed](docs/reproduction/2026-08-11-task5-auth-service-token-replacement-image-passed.md)。

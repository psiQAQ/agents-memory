...
| 阶段 | 客户端 | 协议与状态 |
| --- | --- | --- |
| Stage 1 | Claude Code `2.1.226`、OpenCode `1.18.16`、Pi `0.84.1` | auth/fixture builds Ready；Claude diagnostic host-timeout-during-up Blocked，TUI/real Not Run |
...
**Boundary**：Task 5 root `bfb3839a` fixture fix、Proxy `9e456a5/sha256:55fedae3...` 与 tools `sha256:8ca1a2a8...` 继续为 **Ready（build/assets only）**。历史 Blocked records 保持 append-only；[08c403c6 host-timeout-during-up](docs/reproduction/2026-08-11-task5-diag-claude-p-20260811-08c403c6-host-timeout-during-up-blocked.md) 的首个 workload attempt 在 Compose up 被 host timeout 中止。evidence 为 0 files，bootstrap/config/headless 未执行；当前为 **Blocked / host-timeout-during-up**，不得声称 JSON/classification/root cause。下一步为 cleanup 后的新 tuple、long-lived async exec 与 direct structured JSON parser；后续两写、六读、final、真实 headless、TUI、真实模型与 Codex Stage 2 均为 **Not Run**。旧 Windows + Claude 方案及其运行记录均归档为 **Legacy**。

...
| 阶段 | 客户端 | 协议与状态 |
| --- | --- | --- |
| Stage 1 | Claude Code `2.1.226`、OpenCode `1.18.16`、Pi `0.84.1` | auth/fixture builds Ready；formal Mock 在 Claude write Blocked/diagnostic pending，TUI/real Not Run |
...
**Boundary**：Task 5 root `bfb3839a` fixture fix、Proxy `9e456a5/sha256:55fedae3...` 与 tools `sha256:8ca1a2a8...` 继续为 **Ready（build/assets only）**。历史 [preflight](docs/reproduction/2026-08-11-task5-mock-20260811-preflight-7c1a9e2b-blocked.md)、[protocol/leak](docs/reproduction/2026-08-11-task5-mock-20260811-fixed-8d4802d5-protocol-leak-blocked.md)、[authfix fixture](docs/reproduction/2026-08-11-task5-mock-20260811-authfix-2eae9df1-tool-fixture-blocked.md) 与新的 [toolsfix Claude write](docs/reproduction/2026-08-11-task5-mock-20260811-toolsfix-7e1031af-claude-write-blocked.md) Blocked 均保持 append-only。formal run 的 protocol/leak `24/24` 与 management 已通过，但 step 8 Claude write fail-stop；aggregate/Core/version 仅收窄 Claude `-p`/capture/CLI-before-Proxy 诊断边界，当前为 **Blocked / diagnostic pending**。后续两写、六读、final、真实 headless、TUI、真实模型与 Codex Stage 2 均为 **Not Run**；不得提前进入 TUI/real。旧 Windows + Claude 方案及其运行记录均归档为 **Legacy**。
